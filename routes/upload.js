...
router.post('/', upload.any(), async (req, res) => {
  try {
    ...
    const r2Url = await r2.uploadToR2(path.join(UPLOAD_DIR, file.filename), `uploads/${file.filename}`, file.mimetype);
    res.json({ filename: file.originalname, url: localUrl, r2_url: r2Url||null, r2_synced: !!r2Url, type: file.mimetype.includes('video')?'video':'image' });
  } catch (error) {
...
router.post('/fetch-and-cache', async (req, res) => {
  ...
  try {
    ...
    let ext = '.bin';
    try { const e = path.extname(new URL(url).pathname); if(e&&e.length<=6)ext=e; } catch(e) { const m={'image/jpeg':'.jpg','image/png':'.png','image/webp':'.webp','image/gif':'.gif','video/mp4':'.mp4','video/webm':'.webm'}; ext=m[contentType]||'.bin'; }
    ...
    res.json({ success: true, url: localUrl, r2_url: r2Url||null, r2_synced: !!r2Url, type, recommended_type: type });
  } catch (error) {
